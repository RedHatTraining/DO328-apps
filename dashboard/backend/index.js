...
const checkHistory = (req, resp) => {
    request.post(_const.HISTORY_FQDN, {json: _const.HISTORY_DATA, timeout: 1500}, (error, res, body) => {
        _helper.handleResponse(resp, error, res)
    })
}

const checkCurrency = (req, resp) => {
    request.get(_const.CURRENCY_FQDN, {timeout: 1500}, (error, res, body) => {
        _helper.handleResponse(resp, error, res)
    })
}

const checkExchangeGateway = (req, resp) => {
    request.get(_const.EXCHANGE_FQDN, {timeout: 1500}, (error, res, body) => {
        _helper.handleResponse(resp, error, res)
    })
}

const checkFrontEnd = (req, resp) => {
    request.get(_const.FRONTEND_FQDN, {timeout: 1500}, (error, res, body) => {
        _helper.handleResponse(resp, error, res)
    })
}
